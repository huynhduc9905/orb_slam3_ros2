...
std::optional<std::vector<Ray2>> ScanDeskewer::deskew(
  ...
  for (std::size_t index = 0; index < scan.ranges.size(); ++index) {
    ...
    if (!std::isfinite(range) && !std::isinf(range)) {
      continue;
    }
    ...
    if (std::isinf(range)) {
      const double far = std::numeric_limits<double>::max() / 4.0;
      rays.push_back({origin, {origin.x + far * direction_x, origin.y + far * direction_y}, false});
    } else {
